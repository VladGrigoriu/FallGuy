    ...
    void Update()
    {
        ...
        if(enemyToShoot != null && CheckTargetVisible() && Vector3.Distance(enemyToShoot.transform.position, shootingPoint.position) < searchRadius)
        {
            if(timer > waitingTime){
                ...
                if(Physics2D.Raycast(shootingPoint.position, enemyToShoot.transform.position)){
                    RaycastHit2D hit = Physics2D.Raycast(shootingPoint.position, direction);
                    Draw2DRay(shootingPoint.position, hit.point);
                    enemyToShoot.GetComponent<Enemy>().TakeDamage(40);
                }
